...
if tipo_registro == "Turno de Trabajo":
    st.sidebar.markdown("---")
    accion_turno = st.sidebar.radio("Acción", ["Registrar Entrada", "Registrar Salida"])
    ...
    if accion_turno == "Registrar Entrada":
        ...
        if not turnos_pendientes.empty:
            with st.sidebar.form("form_salida", clear_on_submit=True):
                ...
                h_out = st.time_input("Hora Salida", datetime.strptime("22:00", "%H:%M").time(), key="time_out_fix")
